...
const TeamCard = ({ member, showGithub = false }: { member: TeamMember; showGithub?: boolean }) => {
  return (
    <motion.div
      ...
      className="group relative hud-border hud-glow bg-card/30 overflow-hidden"
    >
      {/* Image */}
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <a 
            href="#" 
            className="w-10 h-10 border border-foreground bg-background/50 backdrop-blur-sm flex items-center justify-center hover:border-accent hover:text-accent transition-colors"
          >
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className="font-display text-lg mb-1 group-hover:text-accent transition-colors transform group-hover:-translate-y-1 duration-300">
          {member.name}
        </h3>
